...
class PatientProfileController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if($validator->passes()){
            ...
            $user->update();
            return redirect()->route('account.profile')->with('success','You have registered successfully :)');
        }else{
    ...
    public function updatePassword(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            if (Hash::check($request->currentPassword, $user->password)) {
                ...
                return redirect()->route('account.profile')->withErrors(['currentPassword' => 'The current password is incorrect'])->withInput();
            }
        } else {
            return redirect()->route('account.profile')->withErrors($validator)->withInput();
        }
    ...
    public function updatePFP(Request $request)
{
    ...
    if ($validator->passes()) {
        ...
        return redirect()->route('account.profile')->with('success', 'Your profile has been updated successfully!');
    } else {
        return redirect()->route('account.profile')->withErrors($validator)->withInput();
    }
}
